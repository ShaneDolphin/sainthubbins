...
drum-and-bass.wav — 319 events over 8 bars, 16.0s, peak 0.79
```
...
A C minor chord on a three-against-eight Euclidean rhythm, then `Late(0.125)`
pushes the whole thing an eighth-note later so it never coincides with the
snare.

`Late` is worth remembering: small offsets are the difference between parts that
collide and parts that interlock.
...
| `Late(0.125)` → `Late(0.0625)` | the stab tightens against the beat |
| remove `Late(0.125)` | the stab collides with the snare |
