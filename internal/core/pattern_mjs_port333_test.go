...
func TestMJS_Port333_StackCatFastSlowFourth(t *testing.T) {
	...
	if len(f.QueryArc(FractionFromInt(0), FractionFromInt(1))) == 0 { t.Fatalf("FastF empty") }
	s := Pure("d").Slow(FractionFromInt(2))
	if len(s.QueryArc(FractionFromInt(0), FractionFromInt(2))) == 0 { t.Fatalf("Slow 2") }
}
func TestMJS_Port333_SignalRangePerlinFourth(t *testing.T) {
	s := Sine().Range(-1, 1)
...
func TestMJS_Port333_ArpChunkDegradeFourth(t *testing.T) {
