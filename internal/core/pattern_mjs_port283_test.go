...
func TestMJS_Port283_SlowFastCompressZoomFourth(t *testing.T) {
...
func TestMJS_Port283_EuclidSqueezeWithValueFourth(t *testing.T) {
	...
	if p.QueryArc(FractionFromInt(0), FractionFromInt(1))[0].Value != 5 {
		t.Fatalf("WithValue 5")
	}
	q := S("bd").Set(Cutoff(800))
	v := q.QueryArc(FractionFromInt(0), FractionFromInt(1))[0].Value.(map[string]any)
	if v["cutoff"] != 800 {
		t.Fatalf("cutoff 800 got %v", v["cutoff"])
	}
}

func TestMJS_Port283_JuxRevBrakHurryFourth(t *testing.T) {
